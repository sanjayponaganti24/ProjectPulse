    ...
    <div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 18 }}>
          {projects.map((proj) => {
            const progress = proj.progress || (proj.status === 'COMPLETED' ? 100 : proj.status === 'ACTIVE' ? 45 : 10)
            return (
              <Card key={proj._id} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Calendar size={14} />
                    <span>
                      Due {new Date(proj.deadline).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                    </span>
                  </div>
...
export function ProjectFormPage({ edit = false }) {
  ...
  useEffect(() => {
    async function init() {
      try {
        const uRes = await api.get('/api/users').catch(() => ({ data: { users: [] } }))
        ...
        setError(err.response?.data?.message || 'Failed to initialize project form.')
      } finally {
        setLoading(false)
      }
    }
  ...
  function toggleMember(userId) {
    setForm((prev) => {
      ...
      return {
        ...prev,
        members: exists ? prev.members.filter((m) => m !== userId) : [...prev.members, userId],
      }
    })
  }
  ...
  if (loading) return <LoadingState message="Loading project configuration..." />
        ...
        <form onSubmit={handleSubmit}>
          ...
          {!edit && (
            <div className="form-group">
              ...
              >
                {usersList.length === 0 ? (
                  <p style={{ fontSize: 12, color: 'var(--text-light)', padding: 6 }}>No other workspace members available.</p>
                ) : (
                  usersList.map((u) => {
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
            ...
            <Button type="submit" disabled={submitting}>
              {submitting ? 'Saving...' : edit ? 'Update Project' : 'Create Project'}
            </Button>
          </div>
        </form>
      </Card>
...
export function ProjectDetailPage() {
    ...
    async function loadProjectDetails() {
      ...
      try {
        const [pRes, tRes, iRes] = await Promise.all([
          ...
          api.get('/api/tasks', { params: { project: id } }),
          api.get('/api/issues', { params: { project: id } }),
        ])
  ...
  const isManager = user?.role === 'PROJECT_MANAGER' && project.manager?._id === user?.id
  const completedTasks = tasks.filter((t) => t.status === 'COMPLETED').length
  const progressPct = tasks.length ? Math.round((completedTasks / tasks.length) * 100) : (project.progress || 0)
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        title={project.name}
        description={project.description || 'Overview of scope, deliverables, and assignments.'}
        actions={
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Button size="sm" icon={Plus} onClick={() => navigate('/tasks/new')}>
              Add Task
            </Button>
          </div>
        }
      ...
      <div className="stat-grid" style={{ marginBottom: 20 }}>
        <Card style={{ padding: 16 }}>
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Project Manager</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
            <Avatar name={project.manager?.name || 'Manager'} avatar={project.manager?.avatar} size="sm" />
            <span style={{ fontSize: 13, fontWeight: 600 }}>{project.manager?.name}</span>
          </div>
        </Card>

        <Card style={{ padding: 16 }}>
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Timeline Target</span>
        ...
        <Card style={{ padding: 16 }}>
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Tasks Delivered</span>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#0f172a', marginTop: 8 }}>
        ...
        <Card style={{ padding: 16 }}>
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Blockers Logged</span>
          <div style={{ fontSize: 14, fontWeight: 600, color: issues.length ? '#ef4444' : '#10b981', marginTop: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 20 }}>
        ...
        <Card>
          <div className="card-header">
            <div>
              ...
              <p className="card-subtitle">Deliverables scheduled under this initiative</p>
            </div>
            <Link to="/tasks" style={{ fontSize: 12, fontWeight: 600, color: 'var(--primary)' }}>
              Open Board
            </Link>
          </div>

          {tasks.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>No tasks linked to this project yet.</p>
          ) : (
              ...
              {tasks.map((t) => (
                ...
                >
                  <div style={{ minWidth: 0, flex: 1, marginRight: 10 }}>
                    <Link to={`/tasks/${t._id}`} style={{ fontSize: 13, fontWeight: 600, display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {t.title}
                    </Link>
        ...
        <Card>
          <div className="card-header">
            <div>
              <h3 className="card-title">Project Contributors ({project.members?.length || 0})</h3>
              <p className="card-subtitle">Active engineers and collaborators</p>
            </div>
            {isManager && (
              <Link to={`/projects/${id}/settings`} style={{ fontSize: 12, fontWeight: 600, color: 'var(--primary)' }}>
                Manage
              </Link>
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 8, background: '#f8fafc', borderRadius: 8 }}>
              <Avatar name={project.manager?.name} avatar={project.manager?.avatar} size="sm" />
              <div>
                <strong style={{ fontSize: 13, display: 'block' }}>{project.manager?.name}</strong>
                <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{project.manager?.email} • Project Lead</span>
              </div>
            </div>

            {project.members?.map((m) => (
              <div key={m._id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 8px' }}>
                <Avatar name={m.name} avatar={m.avatar} size="xs" />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <span style={{ fontSize: 13, fontWeight: 500, display: 'block' }}>{m.name}</span>
                  <span style={{ fontSize: 11, color: 'var(--text-light)' }}>{m.email}</span>
                </div>
...
export function ProjectSettingsPage() {
  ...
  async function handleRemoveMember(memberId) {
    ...
    setSuccess('')
    try {
      const res = await api.delete(`/api/projects/${id}/members/${memberId}`)
      setProject(res.data.project)
  ...
  async function handleDeleteProject() {
    if (!window.confirm('Are you absolutely sure you want to permanently delete this project? All associated tasks will remain orphaned.')) return
    try {
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto' }}>
      ...
      {error && (
        <div style={{ background: '#fef2f2', border: '1px solid #fee2e2', color: '#991b1b', padding: '10px 14px', borderRadius: 8, fontSize: 13, marginBottom: 16 }}>
          {error}
        </div>
      )}
      {success && (
        <div style={{ background: '#ecfdf5', border: '1px solid #d1fae5', color: '#065f46', padding: '10px 14px', borderRadius: 8, fontSize: 13, marginBottom: 16 }}>
          {success}
        </div>
      )}
      ...
      <Card style={{ marginBottom: 24 }}>
        <div className="card-header">
          <div>
            ...
            <p className="card-subtitle">Invite workspace members to collaborate on this initiative</p>
          </div>
        </div>
      ...
      <Card style={{ borderColor: '#fca5a5', background: '#fffafa' }}>
        <div className="card-header">
          <div>
            <h3 className="card-title" style={{ color: '#b91c1c' }}>Danger Zone</h3>
            <p className="card-subtitle">Irreversible actions regarding this project</p>
          </div>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20 }}>
          <div>
            <strong style={{ fontSize: 13, color: '#0f172a', display: 'block' }}>Delete this project</strong>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: '4px 0 0' }}>
              Permanently remove this project and unlink all associated activity.
            </p>
          </div>
