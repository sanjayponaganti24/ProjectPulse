                          ...
                          style={{
                            ...
                            background:
                              t.status === "COMPLETED"
                                ...
                                : "transparent",
                            borderColor:
                              t.status === "COMPLETED" ? "#10b981" : "#94a3b8",
                            display: "grid",
                            placeItems: "center",
                            color: "#fff",
                            fontSize: 10,
...
export function TaskFormPage({ edit = false }) {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        setError(
          err.response?.data?.message || "Failed to initialize task form.",
        );
      } finally {
        setLoading(false);
      }
  ...
  async function handleSubmit(e) {
    ...
    if (!form.project || !form.assignedTo) {
      setError(
        "Please select both a project and an assigned member from the project.",
      );
      return;
    }
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      <Card>
        ...
        >
          {task.description ||
            "No detailed specifications or acceptance criteria were written for this task."}
        </p>
      </Card>
    </div>
  );
