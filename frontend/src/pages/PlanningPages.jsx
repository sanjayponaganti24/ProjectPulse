...
export function MilestonesPage() {
  ...
  if (loading) return <LoadingState message="Calculating roadmap milestones..." />
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        {targetProjects.map((p) => {
          ...
          const pct = projTasks.length ? Math.round((completedCount / projTasks.length) * 100) : (p.status === 'COMPLETED' ? 100 : 30)

          const milestones = [
            {
              name: 'Architecture & Inception Spec',
              targetDate: new Date(new Date(p.startDate).getTime() + 10 * 24 * 60 * 60 * 1000).toLocaleDateString(),
            ...
            },
            {
              ...
              targetDate: new Date(new Date(p.startDate).getTime() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString(),
              status: pct >= 60 ? 'COMPLETED' : pct >= 20 ? 'IN_PROGRESS' : 'TODO',
              desc: 'Primary user flows, API integrations, and internal dogfooding build.',
            },
            {
              name: 'QA Hardening & Security Audit',
              targetDate: new Date(new Date(p.deadline).getTime() - 10 * 24 * 60 * 60 * 1000).toLocaleDateString(),
              status: pct >= 90 ? 'COMPLETED' : pct >= 60 ? 'IN_PROGRESS' : 'TODO',
            ...
            <Card key={p._id}>
              <div className="card-header">
                <div>
                  ...
                  <Link to={`/projects/${p._id}`} className="card-title" style={{ fontSize: 17 }}>
                    {p.name}
                  </Link>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--primary)' }}>{pct}% Complete</span>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14, marginTop: 16 }}>
                {milestones.map((m, idx) => (
                  ...
                  >
                    ...
                    <div style={{ marginTop: 'auto', paddingTop: 6, fontSize: 11, color: 'var(--text-light)', display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <Card key={sprint.id}>
            <div className="card-header">
              <div>
                ...
                <h3 className="card-title" style={{ fontSize: 17 }}>{sprint.title}</h3>
                <p className="card-subtitle" style={{ marginTop: 4 }}>{sprint.goal}</p>
              </div>
              <div style={{ textAlign: 'right' }}>
            ...
            {sprint.tasks.length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--text-muted)', padding: '10px 0' }}>No tasks in this sprint.</p>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 10, marginTop: 12 }}>
                {sprint.tasks.slice(0, 4).map((t) => (
                  ...
                  >
                    ...
                    <Link to={`/tasks/${t._id}`} style={{ fontSize: 13, fontWeight: 600, color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {t.title}
                    </Link>
