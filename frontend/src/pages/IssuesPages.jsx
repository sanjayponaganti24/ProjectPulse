              ...
              {filteredIssues.map((iss) => (
                <tr key={iss._id}>
                  ...
                  <td>
                    ...
                    >
                      <Avatar
                        name={iss.reportedBy?.name || "User"}
                        avatar={iss.reportedBy?.avatar}
                        size="xs"
                      />
...
export function IssueFormPage({ edit = false }) {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        setError(
          err.response?.data?.message || "Failed to initialize issue form.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const assignableMembers = selectedProject
    ? [selectedProject.manager, ...(selectedProject.members || [])].filter(
        Boolean,
      )
    : [];
  const projectTasks = tasksList.filter(
    (t) => t.project?._id === form.project || t.project === form.project,
  );
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      <Card>
        ...
        >
          {issue.description ||
            "No additional notes or reproduction steps provided."}
        </p>
      </Card>
    </div>
  );
