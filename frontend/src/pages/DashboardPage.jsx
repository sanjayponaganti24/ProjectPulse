              ...
              <ul className="deadline-list">
                {upcomingDeadlines.map((task) => (
                  <li
                    key={task._id}
                    className={
                      task.isOverdue ? "deadline-item overdue" : "deadline-item"
                    }
                  >
                    ...
                    <Badge tone={task.isOverdue ? "high" : task.priority}>
          ...
          {metrics.openIssues.length === 0 ? (
            ...
            <div className="activity-list">
              {metrics.openIssues.slice(0, 5).map((issue) => (
                <Link
                  to={`/issues/${issue._id}`}
                  className="activity-row dashboard-issue-row"
                  key={issue._id}
                >
          ...
          {recentActivity.length === 0 ? (
            ...
            <ul className="activity-feed">
              {recentActivity.map((event) => (
                <li
                  key={event.id}
                  className={`activity-feed-item activity-${event.tone}`}
                >
